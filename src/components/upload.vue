<template>
  <div class="userWrap">
    <Row>
      ...
      <Col span="21">
      <p style="color: #666666;margin-bottom: 20px;">支持jpg/png格式，{{uploadData.type}}单张建议规格（{{uploadData.size}}，大小<10M），最多上传6张图片。</p>
      <div>
              ...
              <vddl-list class="panel__body--list" :list="uploadList" :horizontal="false">
                <vddl-draggable class="panel__body--item itemWrap" v-for="(item, index) in uploadList" :key="item.url"
                  ...
                  <div @click="uploadClick(index)" style="overflow: hidden;">
                    <div :class="['dargColor', {active: bgRed[index]}]"><Icon type="arrow-swap" class="icon-jiantou"></Icon></div>
                    <div style="float:left;margin-top: 15px;">
                      <Upload
                        ...
                        :action="`http://upload.dreamhiway.com/uploadimg?key=job&t=${uploadData.size}`">
                        <div class="imgWrap" v-if="!item.url">
                          <Icon type="ios-cloud-upload" size="30" style="margin-top: 10px;" color="#2d8cf0"></Icon>
                    ...
                    <div class="uploadAction">
                      <span v-if="uploadList.length > 1" @click="minusUpload(index)"><Icon color="#999" size="26" type="ios-minus"></Icon></span>

                      <span v-if="uploadList.length < 6" @click="plusUpload(index)"><Icon color="#999" size="26" type="ios-plus"></Icon></span>
                    </div>
                  </div>
                </vddl-draggable>
              </vddl-list>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getData (types = this.uploadData.type) {
        this.$http.post(apis.AdvertsList, { type: types })
          .then((res) => {
            res.json().then((ress) => {
              if (ress.success) {
